...
def tcl_filesystem_path(path: str) -> str:
    """Normalise *path* for embedding in generated TCL (preserve ``/mnt/…``)."""
    text = (path or "").strip().replace("\\", "/")
    if not text:
        return text
    # Absolute POSIX / WSL paths must not go through Windows Path.resolve().
    if text.startswith("/"):
        return text
...
def build_osvvm_batch_script(
    ...
    *,
    ...
    startup_q = tcl_quote(tcl_filesystem_path(startup_tcl))
    pro_q = tcl_quote(tcl_filesystem_path(pro_file))
...
def prepare_osvvm_run(
    *,
    ...
    When *for_wsl* is true, paths written into the TCL script use ``/mnt/…``
    form and Windows ``which`` shims are skipped (Linux ``tclsh`` inside WSL).
    """
    from ghdl_studio.tool_backend import windows_path_to_wsl
    ...
    if for_wsl:
        startup_s = windows_path_to_wsl(str(startup))
        pro_s = windows_path_to_wsl(str(pro_path))
        ghdl_s = _ghdl_name_for_wsl(ghdl_executable)
        content = build_osvvm_batch_script(
            startup_s,
            pro_s,
            ghdl_executable=ghdl_s,
            windows_shim_dir=None,
        )
    else:
...
def _ghdl_name_for_wsl(ghdl_executable: str | None) -> str | None:
    """Map a Settings GHDL path to a Linux-friendly name/path for WSL TCL."""
    raw = (ghdl_executable or "").strip()
    if not raw:
        return None
    posix = raw.replace("\\", "/")
...
def build_osvvm_precompile_script(
    ...
    *map_path*, when set, rewrites absolute host paths (e.g. to ``/mnt/c/…``)
    before they are quoted into the script. *startup_tcl* / *library_directory*
    must be real host paths that exist (``map_path`` only affects the script text).
    """
    if not startup_tcl:
        raise ValueError("OSVVM StartUp.tcl path is required.")
    ...
    startup_path = Path(startup_tcl).expanduser().resolve()
    ...
    lib_resolved = Path(lib_dir).expanduser().resolve()

    def _map(path: str) -> str:
        return map_path(path) if map_path else path
    ...
    mcode_guard = build_osvvm_mcode_coverage_guard_tcl()
    startup_q = tcl_quote(tcl_filesystem_path(_map(str(startup_path))))
    lib_q = tcl_quote(tcl_filesystem_path(_map(str(lib_resolved))))
    target_q = tcl_quote(tcl_filesystem_path(_map(str(build_target.resolve()))))
    ...
    *,
    ...
    if for_wsl:
        content = build_osvvm_precompile_script(
            ...
            target=target,
            ghdl_executable=_ghdl_name_for_wsl(ghdl_executable),
            windows_shim_dir=None,
            map_path=windows_path_to_wsl,
        )
    else:
        shim_dir = install_windows_osvvm_shims(
            directory / "ghdl_studio_which_shim",
